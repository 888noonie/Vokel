...
export const WaveformVisualizer: React.FC<WaveformVisualizerProps> = ({ status, volume = 0 }) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      } else if (status === "capturing_vision") {
        primaryColor = "rgba(244, 63, 94, 0.8)"; // Rose camera capture cue
  ...
  return (
    <div className="vokel-panel-subtle rounded-2xl p-4 h-32 sm:h-40 flex flex-col justify-between overflow-hidden">
